...
for (let pijet = 0; pijet < mahasiswa.length; pijet++) {
    if (mahasiswa[pijet].postgraduate) {
        postGraduate.push(
            {
                name: mahasiswa[pijet].name,
                mastery: mahasiswa[pijet].mastery
            }
...
for (let dayat = 0; dayat < postGraduate.length; dayat++) {
    console.log("Nama: " + postGraduate[dayat].name + "\nKeahlian: " + postGraduate[dayat].mastery);
